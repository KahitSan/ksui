...
| Two or more non-voided time-bound lines share the same transaction + client (package_id not part of the key). | They combine into one "availment group" (a single bucketed end used by the board) whenever the run is CONTINUOUSLY OCCUPIED — a chain breaks only when a line starts after the RUNNING MAXIMUM end of every prior sibling in the chain so far (a frontier, not just the immediately preceding line), so a shorter line nested inside a longer still-covering one — or a sibling with a NULL end — can never falsely reset the frontier and split a genuinely continuous run. Equal-start, equal-boundary, and overlapping lines all continue the same chain. Ordering within the partition is a total order — `(started_at, ends_at, id)` — so tied `started_at` values group deterministically. A pair that has a real gap between them (e.g. two independently-booked, non-adjacent stays) each keep their own real window and their own board section. | Matches every real "one booking, multiple lines" shape: `/extend` (including a cross-package continuation — the new variant need not belong to the source's package), Add-another-block continuation, a base line + same-charge extension sharing one transaction-fixed `NOW()` started_at, AND a same-transaction add-on nested inside a longer line's still-running window (comparing only against the immediately preceding sibling's end — via `LAG` — wrongly reset the frontier on both of these shapes, vanishing the earlier line from the board mid-session). Summing any same-client group's durations regardless of adjacency (no gap check at all) wrongly merged independent bookings and made some vanish from the board. | `routes-line-items.ts` `availment_chain_flags`/`availment_chain_ids`/`availment_groups` (frontier `MAX(ends_at)` over `ROWS BETWEEN UNBOUNDED PRECEDING AND 1 PRECEDING`, package_id-free key) | `combined-stay-independent-bookings.test.ts` |

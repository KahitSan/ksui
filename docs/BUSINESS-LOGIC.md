...
This is a plain-language map of every rule the finance plugin enforces — what happens to your
data (receipts, payments, accounts, subscriptions), not how the code is written. If you manage
the front desk, run reports, or make a call about how money should be handled, this is written
for you; no engineering background needed.

Each **Feature** section starts with one sentence on what the feature is for, then a table of
**Scenarios** — a real situation, in "if this happens, then that happens" form. Every row
includes the *why* behind the rule where one is known, plus two columns for the engineering
team: where it's enforced in the code, and which automated test proves it still works. Guard
and error cases (things that get rejected) are listed as their own rows, not buried in footnotes
— a rejection is a business rule too.

The **Open questions** section at the end lists everything the verification pass could not
confirm from the code and tests alone. These are the exact items worth an answer from whoever
owns the business rule — some may be "yes, that's intentional," others may be real gaps.

Each **Feature** also carries a **"Last verified"** stamp. That date records when the feature's
rows were last re-derived from source by an adversarial verification pass — not when the
underlying code last changed. How stale a feature actually is (has the cited code or its tests
moved since that pass) is a derived fact, not something hand-maintained here — see the
**Freshness** appendix, generated by `node scripts/doc-freshness.mjs`.

## Verification

This document was independently re-verified on 2026-07-19 by five agents working blind to each
other's output: three adversarial verifiers, each re-deriving a slice of the table above
directly from source (`verify-charge`: Features 1, 4, 5, 6, 7, 8; `verify-edit`: Features 2, 3,
4; `verify-other`: Features 9–20), and two blind inventories built with zero visibility into
this document — one enumerating every asserted behavior found by reading the plugin's route/lib
code (`enum-code`), one enumerating every asserted behavior found by reading the plugin's test
suite (`enum-tests`). The two inventories were then diffed against this document's scenario
tables to find behaviors the original mapping pass missed entirely.

- **Rows confirmed as originally written:** the large majority — every feature section not
  called out below checked out against both code and (where cited) test assertions.
- **Rows corrected:** 11 — 5 wrong code-line citations (pointing at a comment or an unrelated
  block instead of the actual enforcement), 1 wrong business-rule claim (a subcategory
  "active-list" check that doesn't actually filter on active), 1 mischaracterized formula (EWT
  rounding, flagged as suspicious but is algebraically the standard round-to-cents order), 1
  unverifiable-without-UI-code claim now flagged as such, 1 resolved open question that a
  verifier proved false (the single-payer DB constraint test genuinely contains the assertion in
  question), and 2 rows given test citations they were missing.
- **Rows added:** 3 new feature sections the original pass never reached (`GET /outstanding`,
  CSV export, and the live-board SSE stream — all real HTTP surface with no existing scenario
  coverage), plus corrections/new rows folded into Features 2, 3, 11, 13, 15, 18, and 20.
- **Rows still unverified:** 1 (Feature 18's "balance computation unavailable → dash" row —
  needs a UI-code check this pass didn't do; flagged inline as `⚠ unverified`).
- New open questions raised by the verifiers are folded into **Open questions** below (items
  15–18 in the current numbering); two previously-open questions covering the single-payer DB
  rule and the `/extend`/`/charge-overage` test coverage were resolved with citations during
  this pass and have since been pruned from the list — their resolutions live in Features 20
  and 3 respectively.
...
**What it does:** Turns a cart — line items, an optional multi-customer split, an optional
discount code, an optional custom start time — into one committed sale, its line items, and
(usually) a payment, all in one save. Route: `POST /charge`.
...
| A voucher code is entered and the cart clears its minimum purchase. | Discount applied (percentage/fixed/free, capped, never negative) to the sale total; response confirms the code and discount. | — | `run-charge.ts:107-134`, `voucher-discount.ts` | — |
...
| A voucher code doesn't resolve. | The system double-checks whether the vouchers add-on itself is simply not installed (in which case it proceeds at full price and says so) versus the code genuinely being invalid (in which case it's rejected). | A missing add-on and a bad code must never look the same to the cashier. | `run-charge.ts:109-121`, `probe-voucher.ts:10-11` | No test found isolating this branch for `/charge` specifically — Open Q2 |
...
| A customer group has its own discount code. | Discount is computed against just that group's own subtotal, not the whole cart, and rolled into the receipt's total. A code that fails to resolve is quietly dropped rather than blocking the sale. | — | `run-charge.ts:140-192` | — |
| A cart has 2 or more customer groups. | The receipt gets a shared batch code so staff can spot linked receipts at a glance; a single-customer sale never gets one. | Lets staff recognize a group booking on sight. | `run-charge.ts:267-277` | — |
...
| A sale is dated in the past. | Flagged as backdated; the backdate reason is only kept when it actually is backdated; its line items start already-completed instead of live. | A backdated sale represents something that already happened — it shouldn't show a live countdown. | `run-charge.ts:293-298,403-406` | — |
...
Last verified: 2026-07-19 (adversarial pass) — open gaps: Q5

**What it does:** Lets staff edit a receipt after checkout — remove or shrink items, add new
items (to an existing customer or a brand-new one), and/or reassign who's paying — as one
save on the SAME receipt. It never creates a second receipt. Route: `POST /:id/apply-cart-edit`.
...
| Scenario | Business rule | Why | Enforced at | Pinned by test |
|---|---|---|---|---|
| Two edit calls hit the same receipt at the same instant. | The receipt is locked first; the second call waits for the first to finish, then simply reads back the first call's already-saved result instead of racing it. | Prevents two saves from double-applying or corrupting each other. | `:264-269` | `cart-reduction-concurrent-idempotency.test.ts` (real two-connection test — one mutation, both callers see the same result) |
...
Last verified: 2026-07-19 (adversarial pass)

**What it does:** Adds a new line item to an ongoing or just-finished rental — `/extend`
continues the same stay forward, `/charge-overage` bills for time that's already elapsed past
a booked end. Both update the receipt's total (and any discount) in the same save.
...
| An overage charge is booked. | It's recorded as already-done (not a live countdown), starting exactly when the original booking's window ended. The original booking itself is left exactly as it was — staff close it out separately. | The overage window is already in the past; it's booked as done from the start. | `:26-28` | — |
...
| Either route inserts a new segment. | The new segment automatically inherits the exact same customer and customer-group attribution as the segment it extends — never re-typed or re-picked. | — | — | `extend-attribution-inheritance.test.ts` covers `/extend` for both the omitted-anchor and explicit-chain cases; code-verified for `/charge-overage` too (both routes pass `src.client_id`/`src.customer_group_id` through to the insert) but `/charge-overage` has no dedicated attribution test of its own |
...
**What it does:** Removes exactly one item from a receipt (as distinct from cart-edit's
bulk reduce-by-item-type), adjusts the receipt's total, and logs the action. Route:
...
Last verified: 2026-07-19 (adversarial pass) — open gaps: Q8

**What it does:** Governs the end-of-life states for a whole receipt — a reversible cancel
(with or without a paper trail), and a permanent write-off of an unpaid balance.
...
appears — the list, the detail view, and the export. It is never stored as its own column;
it's worked out fresh every time from the receipt's current state.
...
A receipt's balance and payment status are always worked out live from these records — never
...
Last verified: 2026-07-19 (adversarial pass) — open gaps: Q3

**What it does:** Three narrow, reason-required fixes let front-desk staff correct a live
receipt without opening the full edit flow: swap who's in the customer pool, reschedule a
customer group's start time, or reassign a group's billed-to client.
...
privacy-scoped, enriched with computed status, balance, item summary, and names resolved
from other plugins. Route: `GET /api/transactions`.
...
Last verified: 2026-07-19 (adversarial pass) — open gaps: Q6

**What it does:** The full drill-down on one receipt — items, payments, customer groups,
client pool, edit history, attachments, payee, and who it's shared with. Route:
...
Last verified: 2026-07-19 (adversarial pass) — open gaps: Q10

**What it does:** Handles income, expense, transfer, and payable entries typed in directly
(not through the POS). Covers create and edit for these, plus VAT, withholding tax, and
...
| Any entry is saved with a tax setting. | Tax is computed one of four ways: inclusive (back the tax out of the total), exclusive (add tax on top, bumping the stored total up), exempt, or non-VAT (no tax) — fixed at 12%. | — | `:373-391` | — |
| An entry has withholding tax turned on. | The rate must be between 0 and 100; the withheld amount is `round(amount × rate) / 100`. | This is the standard round-to-cents formula, not a nonstandard order — algebraically identical to "divide the rate by 100 first, then round," since multiplication/division associate. Re-derived and confirmed during verification; no accounting risk. | `:394-405` (create), `:750-777` (edit) | No isolated unit test found for the formula alone |
...
Last verified: 2026-07-19 (adversarial pass) — open gaps: Q15, Q16
...
Last verified: 2026-07-19 (adversarial pass) — open gaps: Q7, Q9, Q17
...
Last verified: 2026-07-19 (adversarial pass) — open gaps: Q11

**What it does:** Groups every day/month-length item a client has bought under one package
lineage into a "subscription" view, and offers a one-click renew that continues the same
lineage forward.
...
**What it does:** Answers, for the packages plugin's own promo rules (e.g. "first-time
customer" discounts), whether a client already availed a given package lineage before a cutoff
date. Exposed as a cross-plugin lookup, not an HTTP route.
...
Last verified: 2026-07-19 (adversarial pass) — open gaps: Q18 — 1 row unverified
...
| An account's balance is computed. | Counts every payment credited to it, PLUS anything else routed the older way — but a sale is never counted through both paths at once; voided entries never count in either. | — | `account-balances.ts:5-27` | Reconciled byte-for-byte against a 300k-row synthetic dataset — `account-balances.reconcile.test.ts` |
...
Last verified: 2026-07-19 (adversarial pass) — open gaps: Q12
...
Last verified: 2026-07-19 (adversarial pass) — open gaps: Q13, Q16

**What it does:** The cross-cutting promise underneath every feature above: one business's
data never touches another's, checked twice — once in application logic, once at the database
level.
...
| Workspace A tries to read, update, or delete workspace B's receipt/payment/account, including via a raw internal query path. | Fails at every layer — reads return nothing, writes affect zero rows, even a deliberately-forged workspace id on an insert lands in the caller's own workspace. | — | — | `transactions.leak.test.ts`, `transaction-payments.leak.test.ts`, `financial-accounts.leak.test.ts` |
...
Last verified: 2026-07-19 (adversarial pass) — open gaps: Q4

**What it does:** Answers "who still owes us money" — the list front-desk/collections staff
work from to chase down unpaid or partially-paid receipts. Route: `GET /outstanding`.
Missing entirely from the original mapping pass; added during verification.
...
**What it does:** Lets staff download a filtered slice of receipts as a CSV report — either a
per-day consolidated summary or a full detailed row-per-receipt export — via an async background
job with progress polling. Routes: `POST /export`, `GET /export/:jobId/progress`,
`GET /export/:jobId/download`, `GET /export` (recent jobs). Missing entirely from the original
...
**What it does:** Lets the front-desk board refresh itself the moment another terminal changes
a receipt (charges, voids, extends, forfeits) — without the staff member manually reloading.
Route: `GET /api/transaction-line-items/events` (server-sent events). Missing entirely from the
...
- **Two `dead return;` statements** after an already-executed `return c.json(...)` in
  `charge.ts:154-158` and `analytics.ts:54-76`. Unreachable code, confirmed harmless — no behavior
  difference with or without it.
- **`lib/peers.ts`'s `findAccountsByIds` deliberately excluded from the plugin manifest's
  `requires` list.** This is a plugin-loading/dependency-declaration mechanic (so the loader never
  skips this plugin over an optional peer being absent) — the resulting business behavior
...
- **`lib/active-line-summary.ts`'s formatting details** (em dash vs. hyphen, the
  already-prefixed-description double-labeling guard). Cosmetic string formatting, not a business
  rule — the underlying "recompute the title from active lines" rule is already documented in
  Features 9/10.
...
- **Test-hygiene observations from the blind test inventory** (e.g. `cart-edit-new-group-position.test.ts`
  actually exercising a lower-level helper rather than the HTTP route its filename suggests;
  `transaction-customer-groups-single-payer-index.test.ts` being a single thin assertion). These
  are notes about the test suite's own naming/coverage confidence, not product behavior — no
  scenario row would represent them accurately; flagged here so they aren't silently dropped.
...
This document updates in the same change as the behavior it describes — a PR that changes a
rule listed here updates the matching row (or adds a new one) before it merges, the same way a
kdocs/kinfra change ships alongside the code that makes it true. A rule that drifts from this
document is worse than no document at all, because it reads as confirmed when it isn't.

## Freshness (generated — do not hand-edit; regenerate with `node scripts/doc-freshness.mjs`)

For each feature: when its cited logic/tests last actually changed in git (line-range-accurate
via `git log -L` where the doc cites an exact range, file-level otherwise), the feature's own
"Last verified" stamp, and how many commits have touched any of its cited files since that
stamp. A `0` in the last column means nothing has moved since the pass that produced this
document — it does not mean the rows are correct, only that nothing has had a chance to make
them wrong yet. "citation drifted" in Notes means a cited file or line range no longer resolves
against the current tree and is the strongest signal in this table: it means the row's own
citation, not just its freshness, needs a human look.

| # | Feature | Logic last changed | Tests last changed | Last verified | Commits touching cited files since verified | Notes |
|---|---|---|---|---|---|---|
| 1 | POS Charge | 2026-07-10 | 2026-07-16 | 2026-07-19 | 0 | — |
| 2 | Editing a Paid Receipt (Cart Edit) | 2026-07-18 | 2026-07-18 | 2026-07-19 | 0 | — |
| 3 | Extending a Rental / Billing Overage | 2026-07-16 | 2026-07-16 | 2026-07-19 | 0 | — |
| 4 | Voiding a Single Line Item | 2026-07-16 | 2026-07-16 | 2026-07-19 | 0 | — |
| 5 | Void, Unvoid, Forfeit, Soft-Delete | 2026-07-10 | 2026-07-10 | 2026-07-19 | 0 | — |
| 6 | How "Paid / Partial / Unpaid" Is Decided | 2026-07-18 | no test citations | 2026-07-19 | 0 | — |
| 7 | Payments (Settlement Legs) | 2026-07-10 | 2026-07-10 | 2026-07-19 | 0 | — |
| 8 | Counter Quick-Edits | 2026-07-16 | 2026-07-16 | 2026-07-19 | 0 | — |
| 9 | Transaction List | 2026-07-18 | 2026-07-07 | 2026-07-19 | 0 | — |
| 10 | Transaction Detail | 2026-07-18 | no test citations | 2026-07-19 | 0 | — |
| 11 | Manual Transactions | 2026-07-10 | 2026-06-20 | 2026-07-19 | 0 | — |
| 12 | Sharing & Privacy Controls | 2026-07-10 | no test citations | 2026-07-19 | 0 | — |
| 13 | Subcategories | 2026-06-28 | no test citations | 2026-07-19 | 0 | — |
| 14 | Attachments | 2026-07-10 | no test citations | 2026-07-19 | 0 | — |
| 15 | Analytics & Reporting | 2026-07-07 | 2026-07-07 | 2026-07-19 | 0 | — |
| 16 | Subscriptions & Renewals | 2026-06-28 | no test citations | 2026-07-19 | 0 | — |
| 17 | Package Eligibility Check | 2026-07-09 | 2026-07-09 | 2026-07-19 | 0 | — |
| 18 | Financial Accounts | 2026-07-11 | 2026-07-11 | 2026-07-19 | 0 | 1 row(s) marked ⚠ unverified |
| 19 | Payees | 2026-07-06 | no test citations | 2026-07-19 | 0 | — |
| 20 | Multi-Tenant Isolation Guarantee | 2026-07-16 | 2026-07-16 | 2026-07-19 | 0 | — |
| 21 | Outstanding Balances | 2026-07-06 | no test citations | 2026-07-19 | 0 | — |
| 22 | CSV Export | 2026-07-08 | 2026-07-02 | 2026-07-19 | 0 | — |
| 23 | Live Board Updates | 2026-07-06 | no test citations | 2026-07-19 | 0 | — |

// Source: KahitSan/kserp src/lib/accounts-index.tsx (vendored + adapted for the plugin remote).
//
...
// (read from the host's useActiveOrg()). Degrades gracefully: when the
// financial-accounts plugin isn't deployed the fetch 404s/fails and the index
// stays empty — resolveAccount() then falls back to the type-default glyph.

import { createResource, type Resource } from "solid-js";
import { useActiveOrg } from "@kserp/host-ui";
import type { AvatarAccount } from "../components/AccountAvatar";

interface IndexShape {
  byId: Map<number, AvatarAccount>;
}

async function fetchAccountsIndex(orgId: number | null): Promise<IndexShape> {
  const byId = new Map<number, AvatarAccount>();
  if (orgId == null) return { byId };

  const [activeResult, archivedResult] = await Promise.all([
    fetch("/api/financial-accounts?status=active&limit=500", { credentials: "include" })
      .then(async (r) => (r.ok ? ((await r.json()).data ?? []) : []))
      .catch(() => []) as Promise<Array<AvatarAccount & { name?: string }>>,
...
export function useAccountsIndex(): Resource<IndexShape> {
  if (!sharedResource) {
    ...
    const [data] = createResource(() => activeOrg()?.org_id ?? null, fetchAccountsIndex);
    sharedResource = data;
  }

...
export default function AnalyticsRightRail(props: {
  ...
  return (
    <div class="rounded-lg border border-zinc-800/50 bg-zinc-900/50 p-5 ks-hud-clip-top-left-bottom-right flex flex-col">
      <div class="flex items-center justify-between mb-4 border-b border-zinc-800/60 -mx-5 px-5 pb-3">
        <div class="flex items-center gap-1">
          <button
            ...
            classList={{
              "text-amber-400 border-amber-400": tab() === "accounts",
              "text-zinc-500 border-transparent hover:text-zinc-300":
                tab() !== "accounts",
            }}
          >
            Accounts
            <span class="text-[9px] tracking-widest text-zinc-600 normal-case font-medium">
              {liveAccts().length}
            </span>
          </button>
          <button
            ...
            class="px-3 py-1.5 text-[11px] uppercase tracking-[0.2em] font-bold border-b-2 -mb-3 flex items-center gap-2 transition-colors cursor-pointer"
            classList={{
              "text-amber-400 border-amber-400": tab() === "payables",
              "text-zinc-500 border-transparent hover:text-zinc-300":
                tab() !== "payables",
            }}
          >
            ...
            <span class="text-[9px] tracking-widest normal-case font-medium">
              <Show
                when={overdueCount() > 0}
                fallback={
                  <span class="text-zinc-600">{enrichedUpcoming().length}</span>
                }
              >
                <span class="text-red-400">{overdueCount()} late</span>
              </Show>
            </span>
          </button>
        </div>
        <Show when={tab() === "payables" && upcomingTotal() > 0}>
          <div class="text-right">
            <div class="text-xs font-bold tabular-nums text-amber-400">
              {formatCurrency(upcomingTotal())}
            </div>
            <div class="text-[9px] uppercase tracking-widest text-zinc-600">
              due soon
            </div>
          </div>
        </Show>
          ...
          >
            <For each={liveAccts()}>
              {(a) => {
                ...
                return (
                  <div class="flex items-center gap-3 p-2 -mx-2 hover:bg-zinc-800/30 transition-colors">
                    <AccountAvatar account={a} size={32} />
                    <div class="min-w-0 flex-1">
                      <div class="text-xs text-zinc-200 font-medium truncate">
                        {a.name}
                      </div>
                      ...
                      fallback={
                        <div class="text-right shrink-0">
                          <div
                            ...
                            classList={{
                              "text-red-400": capitalOverpaid,
                              "text-amber-400": !capitalOverpaid,
                            }}
                          >
                            {capitalOverpaid
                          ...
                          <div
                            ...
                            classList={{
                              "text-red-400/70": capitalOverpaid,
                              "text-zinc-500": !capitalOverpaid,
                            }}
                          >
                            {capitalLabel}
                          </div>
                ...
                return (
                  <div class="flex items-center gap-3 p-2 -mx-2 hover:bg-zinc-800/30 transition-colors">
                    <div
                      ...
                      classList={{
                        "border-red-500/30": overdue,
                        "border-amber-500/30": dueSoon,
                        "border-zinc-800": !overdue && !dueSoon,
                      }}
                    >
                      <div
                        ...
                        classList={{
                          ...
                          "text-amber-400": dueSoon,
                          "text-zinc-500": !overdue && !dueSoon,
                        }}
                      >
                      ...
                      <div
                        ...
                        classList={{
                          "text-red-300": overdue,
                          "text-amber-300": dueSoon,
                          "text-zinc-200": !overdue && !dueSoon,
                        }}
                      >
                        {t._due.getDate()}
                      </div>
                    ...
                    <div class="min-w-0 flex-1">
                      ...
                      <div class="text-[10px] text-zinc-600 truncate flex items-center gap-1.5">
                        <span class="truncate">{t.payee || "—"}</span>
                        <Show when={overdue}>
                          <span class="text-red-400 font-semibold uppercase tracking-widest shrink-0">
                            · {Math.abs(t._daysOut)}d late
                          </span>
                        </Show>
                        <Show when={dueSoon}>
                          <span class="text-amber-400 font-semibold uppercase tracking-widest shrink-0">
                            · in {t._daysOut}d
                          </span>
                        </Show>
                      </div>
                    </div>
                    <div class="text-sm font-bold tabular-nums text-zinc-200 shrink-0">

// Transactions page — the `/transactions` route of this multi-route plugin.
// Extracted verbatim from index.tsx (now the route dispatcher) so each file
        ...
        >
          ...
          >
            <Show
              ...
              fallback={
                <div
                  class="px-5 sm:px-6 pt-5 pb-4 flex items-start gap-4 border-b border-ks-border/60 shrink-0"
                  data-testid="txn-detail-skeleton-header"
                >
                  <div class="w-12 h-12 shrink-0 animate-pulse rounded bg-ks-fg/5" />
                  <div class="min-w-0 flex-1 space-y-2">
                    <div class="h-3 w-24 animate-pulse rounded bg-ks-fg/5" />
                    <div class="h-5 w-3/4 animate-pulse rounded bg-ks-fg/5" />
                    <div class="h-3 w-32 animate-pulse rounded bg-ks-fg/5" />
                  </div>
                  <button
                    onClick={() => closeDetail()}
                    class="w-10 h-10 flex items-center justify-center text-ks-fg-muted hover:text-ks-fg hover:bg-ks-surface-raised/50 transition-colors shrink-0 ks-hud-clip-button cursor-pointer"
                    aria-label="Close"
                  >
                    <X size={18} />
                  </button>
            ...
            >
              {(() => {
                ...
                return (
                  <div class="px-5 sm:px-6 pt-5 pb-4 flex items-start gap-4 border-b border-ks-border/60 shrink-0">
                    <div
                    ...
                    <div class="min-w-0 flex-1">
                      <p class="text-[10px] tracking-[0.3em] uppercase text-ks-accent font-bold mb-0.5">
                        {editing() ? "Editing" : meta.label} · #{t.id}
                      </p>
                      <h2 class="text-base sm:text-lg font-bold text-ks-fg leading-snug truncate">
                        {editing() ? "Edit transaction" : t.description}
                      </h2>
                      <p class="text-xs text-ks-fg-muted mt-0.5">
            ...
            >
              <div class="mx-5 sm:mx-6 mt-4 rounded-lg border border-ks-danger/30 bg-ks-danger/10 p-4">
                <p class="text-sm text-ks-danger mb-3">
            ...
            <div class="flex-1 overflow-x-hidden overflow-y-auto px-5 sm:px-6 py-5">
              <Show when={formError() && !editing()}>
                <div
                  class="mb-4 rounded-lg border border-ks-danger/30 bg-ks-danger/10 px-3 py-2 text-sm text-ks-danger"
            ...
            >
              <div class="px-5 sm:px-6 py-4 border-t border-ks-border/60 flex flex-col-reverse sm:flex-row sm:items-center sm:justify-end gap-2 bg-ks-surface shrink-0">
                <Show when={isAdmin()}>
                  <Button
                    intent="danger"
                    variant="clip1"

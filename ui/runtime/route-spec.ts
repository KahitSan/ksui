// Pure builder layer for the Phase-3 UI-spine vertical slice (prototyped here in
// the plugin before lifting to ksui). defineRoute() composes a frozen RouteSpec
// out of small helpers; the adapter (./adapter) folds it DOWN to the existing
// ksui ResourceUiSpec, so ResourcePage stays the one render engine. No solid-js,
// no @kserp/host-ui — this module is data only, like ksui's spec.ts.
import type {
  ColumnTone,
  UiColumnRender,
  UiFilter,
...
/** A column descriptor; `render` is the EXISTING ksui UiColumnRender union, so
...
export function col(
  ...
  const c: RouteColumn = opts.orderable === undefined
    ? { key, title: opts.title, render: opts.render }
    : { key, title: opts.title, orderable: opts.orderable, render: opts.render };
  return Object.freeze(c);
}

// ---- body views (the §5/§8 BodyRenderer seam) ------------------------------

/** A `table` body. `kind` keys the future BodyRenderer registry; today only
...
// ---- header actions (the §9 flow seam) -------------------------------------

/** A header action. `flow` is a STRING id (never an inline fn) so the §9 flow
 *  runtime slots in behind host.runFlow(flowId, ctx) without recontracting. */
...
export function action(
  ...
  return Object.freeze({ id, label: opts.label, flow: opts.flow, ...(opts.icon ? { icon: opts.icon } : {}), ...(opts.variant ? { variant: opts.variant } : {}) });
}
...
/** A route-load setting. Reserved for the §6 settings store; the adapter does
 *  not lower it (it is host-resolved later), so it is presentation metadata only
 *  in this slice. */
...
export const setting = {
  number: (opts: { default: number }): SettingDecl => Object.freeze({ kind: "number", default: opts.default }),
} as const;
...
export interface RouteSpec {
  ...
  readonly view: BodyView | { readonly views: Readonly<Record<string, BodyView>>; readonly default: string };
  readonly form?: FormSpec;
  readonly detail: readonly UiDetailRow[];
  readonly settings?: Readonly<Record<string, SettingDecl>>;
  // Vision §4 sticky-footer slot — reserved in the contract, not rendered this
  // slice (needs an additive host PageShell `footer?` prop).

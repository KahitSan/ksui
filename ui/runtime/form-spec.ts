// Pure builder for the Vision §10 form descriptor (prototyped in-plugin). field.*
// helpers compile DOWN to the EXISTING ksui UiField union, so ResourceForm renders
// them with zero new path. defineForm(...).fields plugs straight into the adapter
// as spec.fields. No solid-js, no host-ui — data only.
import type { FieldTransform, UiField, UiFieldSelectOption } from "@kahitsan/ksui";
...
// Declared-but-unwired kinds: present in the union so the §10 contract is whole,
...
export interface FormSpec {
  ...
  // §9 binding seam: STRING command ids, not hardcoded verbs, so the real flow
  // runtime slots in without recontracting the form.
...
// ---- lowering (consumed by ./adapter) --------------------------------------

/** Fold one FieldDef + its key into the existing ksui UiField. THROWS on an
 *  unwired kind so the table-only subset fails loudly at build/test. */
export function fieldToUiField(key: string, def: FieldDef): UiField {
  switch (def.kind) {
    ...
    default:
      throw new Error(`form-spec: field "${key}" uses unwired kind "${(def as { kind: string }).kind}" (only text/textarea/select are wired in this slice)`);
  }
}

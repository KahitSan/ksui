 ...
 * default (Asia/Manila) — the only timezone any KahitSan workspace uses today. If
 * the kernel later forwards the workspace timezone in the signed identity, swap the
 * constant for that forwarded value; the gate logic is otherwise unchanged.
 */

const ORG_TZ = "Asia/Manila";

/** Today (YYYY-MM-DD) in the project's workspace timezone. Uses Intl with the en-CA
...
/** True when `dateStr` differs from "today" in the workspace's local timezone. A

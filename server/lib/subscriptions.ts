...
type PrivacyClause = (c: Context, params: unknown[], startIdx: number) => string | null;
...
export async function listSubscriptions(
  ...
  const search = (c.req.query("search") as string | undefined)?.trim().toLowerCase();
  const lineageSlug = (c.req.query("lineage_slug") as string | undefined)?.trim();
  const bucketRaw = (c.req.query("status_bucket") as string | undefined)?.trim();
  const sortByRaw = (c.req.query("sortBy") as string | undefined) ?? "latest_ends_at";
  const sortBy = SORTABLE.has(sortByRaw) ? sortByRaw : "latest_ends_at";
  const sortDir = (c.req.query("sortDir") as string | undefined)?.toUpperCase() === "ASC" ? 1 : -1;

...
export function registerPaymentUpdateRoute(router: Hono, ctx: PaymentRouteCtx): void {
...
// visibility route).
export function registerPaymentRoutes(router: Hono, ctx: PaymentRouteCtx): void {
  const { pool, requireAuth, requireWorkspace, requirePermission } = ctx;
  const data = makeDataSurface(pool);

...
// Unit price 500, duration 1 day, so overage cost per quantity=1 call is
...
const TEST_ORG = 3;
// The router's own charge-overage handler only ever touches the `accounts`
// schema; the two seed rows below (packages.packages / packages.vouchers) are
...
beforeAll(async () => {
  ...
  const pkgRes = await db.query<{ id: number }>(
    `INSERT INTO packages.packages (workspace_id, name, type, effective_from, lineage_slug)
     VALUES ($1, 'Overage Test Package', 'daily', CURRENT_DATE, 'overage-test-pkg')
     RETURNING id`,
    [TEST_ORG],
  );
  ...
  const variantRes = await db.query<{ id: number }>(
    `INSERT INTO packages.package_variants
     ...
     VALUES ($1, 'Overage Day Pass', 'standard', 1, 'day', 500.00, 'PHP')
     RETURNING id`,
    [variantPackageId],
  );
  ...
  const voucherRes = await db.query<{ id: number }>(
    `INSERT INTO packages.vouchers (workspace_id, code, type, value)
     VALUES ($1, 'OVERAGE20', 'percentage', 20)
     RETURNING id`,
    [TEST_ORG],
  );
...
/** Inserts a parent transaction + an overdue source line item, returns their ids. */
async function seedOverdueBooking(opts: {
  subtotal: number;
  ...
  const txnRes = await db.query<{ id: number }>(
    ...
    [
      TEST_ORG,
      opts.amount,
      `overage-voucher-test-${Date.now()}-${seedCounter++}`,
      "test-user-id",
      opts.subtotal,
      opts.discountAmount,
  ...
  // ends_at in the past so the row qualifies for charge-overage's overdue check.
  const liRes = await db.query<{ id: number }>(
    `INSERT INTO accounts.transaction_line_items
     ...
     VALUES ($1, $2, $3, 'Day 1', 1, $4, 1, 'day', NOW() - interval '2 days', NOW() - interval '1 day', 'active', $5)
     RETURNING id`,
...
describe("POST /:id/charge-overage re-applies the attached voucher (real Postgres)", () => {
  it("recomputes discount_amount/amount for a voucher-attached customer group", async () => {
    ...
    const res = await request(
      honoApp,
      "POST",
      `/api/transaction-line-items/${lineItemId}/charge-overage`,
      { package_variant_id: variantRowId, quantity: 1 },
    );
